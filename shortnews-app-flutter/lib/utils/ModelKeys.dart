class CommonKeys {
  static String id = 'id';
  static String createdAt = 'createdAt';
  static String updatedAt = 'updatedAt';
}

class NewsKeys {
  static String commentCount = 'commentCount';
  static String content = 'content';
  static String shortContent = 'shortContent';
  static String sourceUrl = 'sourceUrl';
  static String image = 'image';
  static String newsStatus = 'newsStatus';
  static String postViewCount = 'postViewCount';
  static String title = 'title';
  static String newsType = 'newsType';
  static String allowComments = 'allowComments';
  static String categoryRef = 'category';
  static String authorRef = 'authorRef';
  static String caseSearch = 'caseSearch';
  static String categoryName = 'categoryName';
}

class UserKeys {
  static String name = 'name';
  static String email = 'email';
  static String image = 'image';
  static String mobile = 'mobile';
  static String loginType = 'loginType';
  static String isNotificationOn = 'isNotificationOn';
  static String themeIndex = 'themeIndex';
  static String appLanguage = 'appLanguage';
  static String oneSignalPlayerId = 'oneSignalPlayerId';
  static String isAdmin = 'isAdmin';
  static String isTester = 'isTester';
  static String bookmarks = 'bookmarks';
}

class CategoryKeys {
  static String name = 'name';
  static String image = 'image';
}

class AudiencePollKeys {
  static String pollQuestion = 'pollQuestion';
  static String pollDuration = 'pollDuration';
  static String userId = 'userId';
  static String pollChoiceList = 'pollChoiceList';
  static String pollTagsList = 'pollTagsList';
  static String endedAt = 'endedAt';
}

class AudiencePollUserKeys {
  static String pollUserId = 'pollUserId';
  static String pollAnswer = 'pollAnswer';
  static String pollUserName = 'pollUserName';
  static String pollUserImage = 'pollUserImage';
}

class AppSettingKeys {
  static String disableAd = 'disableAd';
  static String disableLocation = 'disableLocation';
  static String disableHeadline = 'disableHeadline';
  static String disableQuickRead = 'disableQuickRead';
  static String disableStory = 'disableStory';
  static String disableSponsored = 'disableSponsored';

  static String termCondition = 'termCondition';
  static String privacyPolicy = 'privacyPolicy';
  static String contactInfo = 'contactInfo';
  static String dashboardWidgetOrder = 'dashboardWidgetOrder';
  static String flutterWebBuildVersion = 'flutterWebBuildVersion';
  static String adType = 'adType';
}

class UserPreferenceCategoryKeys {
  static String categoryId = 'categoryId';
  static String userId = 'userId';
  static String categoryName = 'categoryName';
}
